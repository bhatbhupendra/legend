...
<div class="box-panel">
    <div class="eform-wrapper">
        ...
        <div class="eform-topbar">
            <div>
                <h4 class="eform-title mb-0">
                    {{ isset($product) ? 'Edit Wellway Product' : 'Create Wellway Product' }}
                </h4>
                <div class="eform-subtitle">
                    Manage product details with the same premium UI style as your inventory tables
                </div>
            </div>

            <div class="eform-top-actions">
                <span class="eform-top-badge">
                    {{ isset($product) ? 'Update Mode' : 'Create Mode' }}
                </span>
            </div>
        </div>
        ...
        <div class="eform-infobar">
            ...
            <span class="eform-info-chip"><strong>Form:</strong> Product Master</span>
            @if(isset($product))
            <span class="eform-info-chip"><strong>Status:</strong>
                {{ $product->is_active ? 'Active' : 'Inactive' }}</span>
            @else
            <span class="eform-info-chip"><strong>Opening Stock:</strong> Available on create only</span>
            @endif
        </div>

        <form action="{{ isset($product) ? route('wellway.update', $product->id) : route('wellway.store') }}"
            method="POST">
            @csrf
            @if(isset($product))
            @method('PUT')
                                ...
                                <div class="row">
                                    <div class="col-md-6 eform-field">
                                        ...
                                        <input type="text" name="sku" id="sku" class="form-control eform-control"
                                            ...
                                            placeholder="Enter product SKU" required oninput="updateProductPreview()">
                                    ...
                                    <div class="col-md-6 eform-field">
                                        ...
                                        <input type="text" name="name_jp" id="name_jp"
                                            ...
                                            placeholder="Enter Japanese product name" oninput="updateProductPreview()">
                                    ...
                                    <div class="col-md-6 eform-field">
                                        <label class="eform-label">Hinge</label>
                                        <input type="text" name="hinge" id="hinge" class="form-control eform-control"
                                            value="{{ old('hinge', $product->hinge ?? '') }}"
                                            placeholder="Example: Left, Right"
                                            oninput="updateProductPreview()">
                                        <small class="eform-hint">Use simple CSS-friendly names for preview.</small>
                                        @error('hinge')
                                        <div class="eform-error">{{ $message }}</div>
                        ...
                        <div class="eform-section">
                            ...
                            <div class="eform-section-body">
                                <div class="row">
                                    ...
                                    @if(!isset($product))
                                    <div class="col-md-4 eform-field">
                                        <label class="eform-label">Opening Stock</label>
                                        <input type="number" name="opening_stock" id="opening_stock"
                                            class="form-control eform-control" value="{{ old('opening_stock', 0) }}"
                                            min="0" placeholder="0" oninput="updateProductPreview()">
                                        <small class="eform-hint">Only shown when creating a new product.</small>
                                        @error('opening_stock')
                                        <div class="eform-error">{{ $message }}</div>
                                        @enderror
                                    </div>
                        ...
                        <div class="eform-preview">
                            ...
                            <div class="eform-preview-card">
                                ...
                                <div class="eform-meta">
                                    ...
                                    <span class="eform-pill">
                                        <span class="eform-hinge-dot" id="preview-hinge-dot"
                                            style="background: {{ strtolower(old('hinge', $product->hinge ?? '#d1d5db')) }};"></span>
                                        <span
                                            id="preview-color">{{ old('hinge', $product->hinge ?? 'No Hinge') ?: 'No Hinge' }}</span>
                                    </span>
                                    ...
                                    @if(!isset($product))
                                    <span class="eform-pill">
                                        Opening: <strong id="preview-stock">{{ old('opening_stock', 0) }}</strong>
                                    </span>
                                    @else
                                    <span class="eform-pill">
                                        Status:
                                        <strong>{{ old('is_active', $product->is_active) ? 'Active' : 'Inactive' }}</strong>
                                    </span>
                                    @endif
                                </div>
                            </div>
            ...
            <div class="eform-footer">
                <div class="eform-footer-note">
                    Please review product information before saving.
                </div>
...
function updateProductPreview() {
    ...
    const openingStock = document.getElementById('opening_stock')?.value || '0';
    ...
    if (stockEl) {
        stockEl.textContent = openingStock;
    }
}
</script>
@endsection